...
object RouteUtils {
    ...
    //初始化Bundle参数
    fun initBundle(params: Parcelable) = Bundle().apply {
       this.putParcelable(ARGS, params)
    }
    ...
    fun navTo(
        ...
        if (args!=null) {
            when(args) {
                is Parcelable -> {
                    navCtrl.currentBackStackEntry?.replaceArguments(initBundle(args))
                }
                is String -> {
            ...
            navCtrl.previousBackStackEntry?.arguments = null
            navCtrl.currentBackStackEntry?.arguments = null
        }
