...
fun HamScaffold() {
    ...
    val navBackStackEntry by navCtrl.currentBackStackEntryAsState()//2.获取导航中回退栈的信息
    val currentDestination = navBackStackEntry?.destination //3.获取当前路由
    ...
    Scaffold(
        ...
        bottomBar = {
            when (currentDestination?.route) {//4.获取当前路由的名称
        ...
        content = {
           ...
           // 每个NavController都必须与一个NavHost可组合项相关联
           // 创建并关联NavHost
            NavHost(
                ...
                startDestination = RouteName.MAIN
                 ...
                 navigation(route=RouteName.MAIN,startDestination = RouteName.HOME){
                     //首页
                     composable(route = RouteName.HOME) {
                         HomePage(navCtrl, scaffoldState, homeIndex) {
                             homeIndex = it
                         }
                     }
                     //分类
