...
fun CategoryPage(
    ...
    Box(modifier = Modifier.padding(bottom = BottomNavBarHeight)) {//防止
        Column {
            ...
            HorizontalPager(state = pagerState,dragEnabled=true) { page ->
                onPageSelected(pagerState.currentPage)
                when (page) {
                    0 -> StructurePage(navCtrl) //体系
                    1 -> NaviPage(navCtrl) //导航
                    2 -> PublicAccountPage(navCtrl) //公众号
                }
            }
        }
    }
